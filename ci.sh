...
function do_compress() {
    ...
    if [[ $OS == "archlinux" ]]; then
        file_name="$LLVM_VENDOR_STRING"-Mayuri-clang_"$clang_version"git-archlinux-"$git_hash".tar.xz
    elif [[ $OS == "debian" ]]; then
        file_name="$LLVM_VENDOR_STRING"-Mayuri-clang_"$clang_version"git-bookworm-"$git_hash".tar.xz
    fi
